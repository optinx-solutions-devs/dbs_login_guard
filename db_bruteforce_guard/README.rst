...
1. Every login request is checked against the IP block list.
2. If blocked, access is denied and the event is logged as ``blocked``.
3. If login fails, failure count is recomputed inside the rolling window.
4. On threshold breach, an alert email is sent.
5. On higher threshold breach, the IP is blocked automatically.
